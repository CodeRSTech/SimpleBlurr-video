...
class DetectionHandler:
    def __init__(self, window, coordinator) -> None:
        self._window = window
        ...
        if not self._dont_ask_again and model_name != "None":
            ...
            if dlg.exec() != ModelChangeWarningDialog.DialogCode.Accepted:
                ...
                self._window.set_selected_detection_model(
                    self._coordinator.get_selected_detection_model_name(session_id)
                )
                return
    ...
    def on_detect_current_frame(self, render_fn) -> None:
        ...
        try:
            ...
            self._coordinator.detect_current_frame(session_id)
    ...
    def on_start_background_detection(self) -> None:
        ...
        try:
            self._coordinator.set_active_session(session_id)
            self._coordinator.start_background_detection(session_id)
    ...
    def on_min_confidence_changed(self, value: float, render_fn) -> None:
        ...
        if session_id is None:
            return
        self._coordinator.update_session_settings(session_id, min_detection_confidence=value)
        self._coordinator.apply_filters_to_layer_b(session_id)
        render_fn(session_id)
    ...
    def on_chosen_labels_changed(self, raw: str, render_fn) -> None:
        ...
        self._coordinator.update_session_settings(session_id, chosen_labels=labels)
        self._coordinator.apply_filters_to_layer_b(session_id)
        render_fn(session_id)
