...
class AnnotationService:
    ...
    def get_frame_presentation(self, session_id: str) -> FramePresentationViewModel:
        ...
        items = session.review_frame_items_by_frame_index.get(frame_index, [])
    ...
    def add_manual_frame_item(
        ...
        session.review_frame_items_by_frame_index.setdefault(frame_index, []).append(item)
        logger.info("Added manual item {} to session {} frame {}", item.item_id, session_id, frame_index)
    ...
    def update_manual_frame_item(
        ...
        session = self._sm.get_session(session_id)
        frame_index = session.playback.current_frame_index
        item = self.get_review_frame_item(session_id, item_key)
        if item is None:
    ...
    def move_manual_frame_items(
        ...
        moved = 0
        for item in items:
            if item.item_key not in keys or item.source != "Manual":
                continue
    ...
    def _duplicate_items(
        ...
        source_items = session.review_frame_items_by_frame_index.get(current, [])
        to_duplicate = [i for i in source_items if i.item_key in keys]
        if not to_duplicate:
            return

        target_items = session.review_frame_items_by_frame_index.setdefault(target, [])
        for src in to_duplicate:
            new_item = FrameItemViewModel(
                item_id=f"manual-{session.next_annotation_id}",
                source="Manual",
                label=src.label,
    ...
    def _seed_review_frame_from_raw(session: Session, frame_index: int) -> None:
        if frame_index in session.review_frame_items_by_frame_index:
            return
        raw = session.raw_frame_items_by_frame_index.get(frame_index)
        if raw is None:
            return
        session.review_frame_items_by_frame_index[frame_index] = copy.deepcopy(raw)
