...
class DetectionService:
    ...
    def get_available_detection_models() -> list[DetectionModelItemViewModel]:
        return [
            DetectionModelItemViewModel(model_id=n, display_name=n)
    ...
    def detect_current_frame(self, session_id: str) -> None:
        ...
        session.raw_frame_items_by_frame_index[frame_index] = filtered

        # Copy onto Layer B
        session.review_frame_items_by_frame_index[frame_index] = copy.deepcopy(filtered)
        logger.info(
    ...
    @staticmethod
    def _passes_filter(item: FrameItemViewModel, settings: ProcessingSettings) -> bool:
        """
    ...
    @staticmethod
    def _map_detections_to_review_items(detections: list[DetectionResult], ) -> list[FrameItemViewModel]:
